...
// TestRegisterCompilesSchemas guards every excel.* tool's JSON Schema against
...
func TestRegisterCompilesSchemas(t *testing.T) {
	r := tools.NewRegistry()
	Register(r)
	if got := len(r.List()); got != 6 {
		t.Fatalf("expected 6 excel.* tools registered (runScript + Phase A workflow tools + query + discover), got %d", got)
	}
}
...
// Tools that bypass runPayload (currently none in this package, but possible
// in the future) are skipped — Register-time wiring is the ground truth.
func TestEveryRunPayloadToolHasJSPayload(t *testing.T) {
	...
	for _, tool := range r.List() {
		if !strings.HasPrefix(tool.Name, "excel.") {
			continue
		}
