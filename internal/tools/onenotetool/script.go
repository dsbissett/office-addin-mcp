...
const runScriptSchema = `{
  ...
  "title": "onenote.runScript parameters",
  "description": "Run an arbitrary user-supplied async JS body inside OneNote.run. The body sees 'context' (RequestContext) and 'args' (scriptArgs) and must return a JSON-serializable value.",
  "type": "object",
  "properties": {
    "script":     {"type": "string", "minLength": 1, "description": "JS body executed inside OneNote.run."},
...
// RunScript returns the onenote.runScript tool definition.
...
func runRunScript(ctx context.Context, raw json.RawMessage, env *tools.RunEnv) tools.Result {
	...
	return runPayloadSum(ctx, env, p.Selector(), "onenote.runScript", args, func(_ any) string {
		return "Ran custom OneNote.run script."
	})
}
